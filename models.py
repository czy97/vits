...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        ...
        with torch.no_grad():
            ...
            neg_cent2 = torch.matmul(-0.5 * (z_p **
                                     2).transpose(1, 2).contiguous(), s_p_sq_r)
            # [b, t_t, d] x [b, d, t_s] = [b, t_t, t_s]
            neg_cent3 = torch.matmul(z_p.transpose(
                1, 2).contiguous(), (m_p * s_p_sq_r))
            ...
            attn = monotonic_align.maximum_path(neg_cent, attn_mask.squeeze(
                1)).unsqueeze(1).detach()  # [b, 1, t_t, t_s]
        ...
        # expand prior, [b, d, t_s] --> [b, d, t_t]
        m_p = torch.matmul(attn.squeeze(1), m_p.transpose(
            1, 2).contiguous()).transpose(1, 2).contiguous()
    ...
    def infer(self, x, x_lengths, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
        ...
        m_p = torch.matmul(attn.squeeze(1), m_p.transpose(1, 2).contiguous()).transpose(
            1, 2).contiguous()  # [b, t', t], [b, t, d] -> [b, d, t']
        logs_p = torch.matmul(attn.squeeze(1), logs_p.transpose(1, 2).contiguous(
        )).transpose(1, 2).contiguous()  # [b, t', t], [b, t, d] -> [b, d, t']
